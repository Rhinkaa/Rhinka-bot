...
const words = ['çikolata', 'egehanss', 'odun', 'bomba', 'süt', 'şimşek', 'yıldırım', 'patlat', 'savaş', 'kelime', 'yok et', 'bilmem', 'öldür', 'sonsuzluk', 'gerçek', 'yarrak', 'yagızmaltepe', 'elmasablam',, 'yagızmaltepe',, 'yagızmaltepe',];
...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.bot) return msg.reply('Botlar ile oynayamazsın!');
		...
		try {
			...
			return msg.channel.send(`Hızınız 110 yavaş amk ${winner.first().author} Kazandın!`);
		} catch (err) {
			this.fighting.delete(msg.channel.id);
			throw err;
		}
